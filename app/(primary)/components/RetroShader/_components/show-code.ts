export const title = "Retro Shader";
...
export const cliscript = "add https://www.cursorxui.vercel.app/registry/retroshader.json";
...
export const democode = `import RetroShader from "@/components/retroshader.tsx";';
...
export default function RetroShader() {
  ...
  const [halftoneSize] = useState(4)
  const [contrast] = useState(1.5)
  const [accentColor] = useState("#00d9ff")
  const [mouseRadius] = useState(100)
  const [repulsionStrength] = useState(1.0)
  const [returnSpeed] = useState(0.3)
  const [accentProbability] = useState(0.03)
  const [sizeVariation] = useState(0.3)
  ...
  useEffect(() => {
    ...
    img.src =
      "https://avatars.githubusercontent.com/u/171645986?s=400&u=b9d5cfdb160b2e71d6f9974c75cc44a0b8fd6499&v=4"
  }, [])
